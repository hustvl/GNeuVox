...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        print('[Dataset Path]', dataset_path , now_subject ) 
    ...
    def __getitem__(self, idx):
        ...
        if self.bgcolor is None:
            bgcolor = (np.array([random.random() * 255. ,random.random() * 255. ,random.random() * 255.])).astype('float32')
        else:
