...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_canonical_joints(self):
        cl_joint_path = os.path.join(self.dataset_path, 'canonical_joints_'+self.now_subject+'.pkl')
        with open(cl_joint_path, 'rb') as f:
            cl_joint_data = pickle.load(f)
        canonical_joints = cl_joint_data['joints_'+self.now_subject].astype('float32')
    ...
    def load_train_cameras(self):
        cameras = None
        with open(os.path.join(self.dataset_path, 'cameras_'+self.now_subject+'.pkl'), 'rb') as f: 
    ...
    def load_train_mesh_infos(self):
        mesh_infos = None
        with open(os.path.join(self.dataset_path, 'mesh_infos_'+self.now_subject+'.pkl'), 'rb') as f:   
    ...
    def load_train_frames(self):
        img_paths = list_files(os.path.join(self.dataset_path, 'images'),
                               exts=['.png'],keyword='frame_'+str(self.now_subject))
        # print(img_paths)
