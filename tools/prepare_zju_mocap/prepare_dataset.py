...
def get_mask(subject_dir, img_name):
    ...
    msk_path = os.path.join(subject_dir, 'mask_cihp',
                            img_name)[:-4] + '.png'
    msk_cihp_ori = np.array(load_image(msk_path))
...
def main(argv):
    ...
    out_cihp_dir = prepare_dir(output_path, 'masks_cihp')
    ...
    for idx, ipath in enumerate(tqdm(img_paths)):
        out_name = 'frame_'+subject+'_{:06d}'.format(idx)
        ...
        # load and write mask
        mask,msk_cihp_ori = get_mask(subject_dir, ipath)
        save_image(to_3ch_image(mask), 
                   os.path.join(out_mask_dir, out_name+'.png'))
        save_image(msk_cihp_ori, 
                   os.path.join(out_cihp_dir, out_name+'.png'))
        # write image
